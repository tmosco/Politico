...
describe('Office resource', () => {
  ...
  describe('POST /offices', () => {
    ...
    it('When creating a office fails', () => {
  ...
  describe('PATCH /offices/1/name', () => {
    it('should change the name of the office', () => {
      request(app).patch(`${url}/1/Chairman`).end((err, res) => {
        expect(res.statusCode).to.equal(200);
        expect(res.body).to.deep.equal({
          status: 200,
          data: {
    ...
    it("should return an error when the office id doesn't exists", () => {
      request(app).patch(`${url}/3/Counsellor`).end((err, res) => {
        expect(res.statusCode).to.equal(404);
        expect(res.body).to.deep.equal({
          status: 404,
          error: 'Office not found',
  ...
  describe('DELETE /offices/1', () => {
    it('should delete the office based on the id passed', () => {
    ...
    it('trying to get a deleted office should return 404', () => {
      request(app).delete(`${url}/1`).end((err, res) => {
        expect(res.statusCode).to.equal(404);
        expect(res.body).to.deep.equal({
          status: 404,
    ...
    it('trying to deleta a non existent office should throw a 404 not found', () => {
      request(app).delete(`${url}/1`).end((err, res) => {
        expect(res.statusCode).to.equal(404);
        expect(res.body).to.deep.equal({
          status: 404,
